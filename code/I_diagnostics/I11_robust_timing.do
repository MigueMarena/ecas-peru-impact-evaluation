...
//                    (i)  Cruda:           reg X treat i.$fe_estrato, cluster($cl_ccpp)
//                    (ii) Ajustada:        reg X treat i.$fe_estrato i.mes_enc, cluster($cl_ccpp)
...
foreach v of local outc_vO {
	...
	// (ii) Ajustada
	qui reg `v' asig_ccpp i.$fe_estrato i.mes_enc, cluster($cl_ccpp)
	local c_ii = _b[asig_ccpp]
	local p_ii = 2 * ttail(e(df_r), abs(_b[asig_ccpp] / _se[asig_ccpp]))

	// (iii) Ventana común
	qui reg `v' asig_ccpp i.$fe_estrato if ventana_comun == 1, cluster($cl_ccpp)
